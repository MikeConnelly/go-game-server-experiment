...
export default class KineticScroll {
  ...
  // added to configure settings after initialization
  configure(settings: any) {
    this.settings = {
      kineticMovement: settings.kineticMovement || true,
      timeConstantScroll: settings.timeConstantScroll || 325,
      horizontalScroll: settings.horizontalScroll ||true,
      verticalScroll: settings.verticalScroll || true,
      horizontalWheel: settings.horizontalWheel || true,
      verticalWheel: settings.verticalWheel || false,
      deltaWheel: settings.deltaWheel || 40,
  ...
  canCameraMoveY () {
    ...
    let camBoundH = 0 // added due to typescript
    if (camJson['bounds'])
      camBoundH = camJson['bounds']['height']
  ...
  canCameraMoveX () {
    const cam = this.scene.cameras.main
    const camJson = cam.toJSON()
    let camBoundW = 0 // added due to typescript
    if (camJson['bounds'])
      camBoundW = camJson['bounds']['width']
    let camBoundX = 0
    if (camJson['bounds'])
      camBoundX = camJson['bounds']['x']
